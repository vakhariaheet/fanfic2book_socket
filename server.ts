...
import WattpadUser from './Web Scrappers/Wattpad/WattpadUser';
import { decrypt } from '../restapi/utils/Encryption';
import fs from 'fs';
					...
					const getBook = async (callback: () => void) => {
						if (counter < bookids.length) {
							if ((user.downloads !== 5 && !user.userid) || user.userid) {
								...
								if (book.buffer) {
									books.push({
										filename: `${book.bookInfo.title} by ${book.bookInfo.author}.${extension}`,
										content: Buffer.from(book.buffer as any).toString('base64'),
										size: book.size,
									});
									totalSize += book.size;
								}
